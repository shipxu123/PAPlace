...
template<typename nlp_settings>
IntType NlpGPlacerBase<nlp_settings>::solveWithParams(std::vector<float>& obj_params)
{
    auto stopWatch = WATCH_CREATE_NEW("NlpGPlacer");
    stopWatch->start();
    _calcObjStopWatch = WATCH_CREATE_NEW("GP_calculate_obj");
    this->initProblem();
    this->initPlace();
    this->initOperatorsWithParams(obj_params);
    this->constructTasks();
    this->optimize();
    stopWatch->stop();
    return 0;
...
// template<typename nlp_settings>
// IntType NlpGPlacerBase<nlp_settings>::solveWithPAPlace(std::vector<float>& obj_params)
// {
//     auto stopWatch = WATCH_CREATE_NEW("NlpGPlacer");
//     stopWatch->start();
//     _calcObjStopWatch = WATCH_CREATE_NEW("GP_calculate_obj");
...
template<typename nlp_settings>
void NlpGPlacerBase<nlp_settings>::initOperatorsWithParams(std::vector<float>& obj_params)
{
    auto getAlphaFunc = [&]()
    {
        return _alpha;
...
template<typename nlp_settings>
void NlpGPlacerBase<nlp_settings>::guidePlace()
{
    Eigen::Matrix<double, Eigen::Dynamic, 1> grad = _pap->getGuideGradient();

    for (int i = 0; i < 100; i++)
    {
        std::cout << "*********************************Guide Place Phase*********************************" << std::endl;
    }

    for (IndexType cellIdx = 0; cellIdx < _db.numCells(); ++cellIdx)
    {
        std::cout << grad(plIdx(cellIdx, Orient2DType::HORIZONTAL)) << " " << grad(plIdx(cellIdx, Orient2DType::HORIZONTAL)) << std::endl;
    }

    // Guide the locations of different cells
    for (IndexType cellIdx = 0; cellIdx < _db.numCells(); ++cellIdx)
    {
        _pl(plIdx(cellIdx, Orient2DType::HORIZONTAL)) = _pl(plIdx(cellIdx, Orient2DType::HORIZONTAL)) + grad(plIdx(cellIdx, Orient2DType::HORIZONTAL)) * _scale;
        _pl(plIdx(cellIdx, Orient2DType::VERTICAL))   = _pl(plIdx(cellIdx, Orient2DType::VERTICAL))   + grad(plIdx(cellIdx, Orient2DType::VERTICAL))   * _scale;
    }
}
