...
void IdeaPlaceEx::readInitPlacementFile(const std::string &initPlacementFile)
{
    _initPlacementFile = initPlacementFile;
    std::ifstream inf(initPlacementFile.c_str());
    if (!inf.is_open()) 
    {
        ERR("IdeaPlaceEx::%s: cannot open file: %s \n", __FUNCTION__, initPlacementFile.c_str());
        Assert(false);
        return;
    }
    
    INF("IdeaPlaceEx::%s: Reading initial placement from %s...\n", __FUNCTION__, initPlacementFile.c_str());
    
    std::string line;
    IndexType numCellsSet = 0;
    while (std::getline(inf, line))
    {
        ...
        if (words.size() < 3)
        {
            WRN("IdeaPlaceEx::%s: Invalid line format (expected: cellName xLoc yLoc): %s\n", __FUNCTION__, line.c_str());
            continue;
        }
        
        std::string cellName = words.at(0);
        LocType xLoc = static_cast<LocType>(::atoi(words.at(1).c_str()));
        LocType yLoc = static_cast<LocType>(::atoi(words.at(2).c_str()));

	// Find the cell by name
        IndexType cellIdx = cellIdxName(cellName);
        if (cellIdx == INDEX_TYPE_MAX)
        {
            WRN("IdeaPlaceEx::%s: Cell not found: %s\n", __FUNCTION__, cellName.c_str());
            continue;
        }

        // Print cell information including size
        const auto &cell = _db.cell(cellIdx);

        LocType width    = cell.xHi() - cell.xLo();
        LocType height   = cell.yHi() - cell.yLo();       

        INF("IdeaPlaceEx::%s: Cell %s (idx=%d) Original location (%d, %d), size: width=%d, height=%d\n", 
           __FUNCTION__, cellName.c_str(), cellIdx, cell.xLoc(), cell.yLoc(), width, height);

        // Set the cell location
        _db.cell(cellIdx).setXLoc(xLoc);
        _db.cell(cellIdx).setYLoc(yLoc);
        numCellsSet++;

        width    = cell.xHi() - cell.xLo();
        height   = cell.yHi() - cell.yLo();

        INF("IdeaPlaceEx::%s: Set cell %s (idx=%d) to location (%d, %d), size: width=%d, height=%d\n",
            __FUNCTION__, cellName.c_str(), cellIdx, xLoc, yLoc, width, height);
    }
     
    INF("IdeaPlaceEx::%s: Successfully set initial placement for %d cells\n", __FUNCTION__, numCellsSet);
    inf.close();
}
...
bool IdeaPlaceEx::parseFileBased(int argc, char **argv)
{
    ...
    if (_args.initPlacementFileIsSet())
    {
        INF("IdeaPlaceEx:%s Initial placement file is set: %s \n", __FUNCTION__, _args.initPlacementFile().c_str());
        _initPlacementFile = _args.initPlacementFile();
    }
...
LocType IdeaPlaceEx::solve(LocType gridStep)
{
    ...
    // Print cell information after legalization
    INF("Ideaplace: Cell information after legalization:\n");
    for (IndexType cellIdx = 0; cellIdx < _db.numCells(); ++cellIdx)
    {
        const auto &cell = _db.cell(cellIdx);
        const auto &bbox = cell.cellBBox();
        INF("  Cell[%d]: name=%s, xLoc=%d, yLoc=%d, bbox_width=%d, bbox_height=%d\n", 
            cellIdx, cell.name().c_str(), cell.xLoc(), cell.yLoc(), 
            bbox.xLen(), bbox.yLen());
    }
...
LocType IdeaPlaceEx::solveWithParams(LocType gridStep, std::vector<float>& obj_params, PAPlace& pap)
{
    auto stopWatch = WATCH_CREATE_NEW("IdeaPlaceEx");
    stopWatch->start();
    omp_set_num_threads(_db.parameters().numThreads());
    ...
    placer.setPAPlacer(pap);
    placer.solveWithParams(obj_params);
